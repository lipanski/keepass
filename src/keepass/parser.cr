...
module Keepass
  class Parser
    ...
    private def decrypt(payload : Bytes) : Array(UInt8)
      ...
      raise Error::DecryptionFailed.new(e.message)
    end
    ...
    private def parse_group_node(group_node : XML::Node) : Group
      uuid, name, groups, entries = nil, nil, Array(Group).new, Array(Entry).new
      group_node.children.each do |group_attribute_node|
        ...
        when "Group"
          groups << parse_group_node(group_attribute_node)
        when "Entry"
          entries << parse_entry_node(group_attribute_node)
        end
      end

      Group.new(uuid.not_nil!, name.not_nil!, groups, entries)
    end
